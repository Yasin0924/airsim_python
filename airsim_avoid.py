...
def navigate_v4(client, waypoints, vehicle_name='UAV0', 
    ...
    for wp_idx, waypoint in enumerate(waypoints):
        ...
        # V5.38: 避障方向记忆 - 避免连续选择失效的方向
        last_avoid_left = None  # 上次避障是否向左
        avoid_fail_count = 0    # 连续避障失败次数
        ...
        while True:
            ...
            if min_all_dist < 2.0 and not direction_locked:
                ...
                while yaw_diff < -math.pi: yaw_diff += 2 * math.pi
                # V5.37: 每步30度，覆盖360度
                rotate_step = math.radians(30) if yaw_diff > 0 else math.radians(-30)
                ...
                for i in range(12):
                    ...
                    client.moveByVelocityAsync(0, 0, 0, duration=0.4,
                    ...
                    print(f"    扫描{i+1}/12: 中{scan_center:.1f}m 左{scan_left:.1f}m 右{scan_right:.1f}m")
            ...
            if center_dist < 6.0:
                ...
                # V5.38: 如果连续3次同方向避障失败，强制换方向
                if avoid_fail_count >= 3 and last_avoid_left is not None:
                    prefer_right = last_avoid_left  # 上次向左失败，现在向右
                    print(f"  ↔️ 连续避障失败{avoid_fail_count}次，换方向")
            ...
            if hold_direction is not None and current_time < hold_until:
                ...
                # 保持期内检查前方障碍物（未锁定时）
                    ...
                    print(f"  ⏸️ 保持期发现障碍({center_dist:.1f}m)，悬停扫描(失败{avoid_fail_count}次)")
                    client.hoverAsync(vehicle_name=vehicle_name)
                    time.sleep(0.5)
                ...
                if center_dist < 6.0:
                    ...
                    # V5.40: 前方安全，悬停确认回归方向
                    print(f"  ⏸️ 保持期结束，悬停确认回归方向...")
                    client.hoverAsync(vehicle_name=vehicle_name)
                    time.sleep(0.3)
                    
                    # 计算回归方向
                    return_dir = target_dir * 0.5 + hold_direction * 0.5
                    norm = np.linalg.norm(return_dir)
                    if norm > 0.01:
                        return_dir = return_dir / norm
                    
                    # 检测回归方向是否安全
                    return_yaw = math.atan2(return_dir[1], return_dir[0])
                    client.moveByVelocityAsync(0, 0, 0, duration=0.5,
                        yaw_mode=airsim.YawMode(False, math.degrees(return_yaw)),
                        vehicle_name=vehicle_name)
                    time.sleep(0.4)
                    
                    # 确认回归方向安全
                    check_center, _, check_left, check_right = get_obstacle_info_v4(client, vehicle_name)
                    print(f"    回归确认: 中{check_center:.1f}m 左{check_left:.1f}m 右{check_right:.1f}m")
                    
                    if check_center > 4.0 and check_left > 2.0 and check_right > 2.0:
                        # 安全，执行回归
                        final_dir = return_dir
                        hold_direction = None
                        avoid_fail_count = 0
                        safety_check_until = current_time + 1.0
                        status = "渐进回归"
                    else:
                        # 不安全，继续保持
                        print(f"    ⚠️ 回归方向不安全，继续保持")
                        hold_until = current_time + 1.0
                        final_dir = hold_direction
                        status = f"延长保持(回归不安全)"
            ...
            # V5.39: 锁定期使用固定方向和速度
            if direction_locked and hold_direction is not None:
                velocity_2d = hold_direction * 1.0  # 固定1m/s
                status = f"保持飞行({hold_until - time.time():.1f}s)"
